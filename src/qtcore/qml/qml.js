...
function connectSignal(item, signalName, value, objectScope, componentScope) {
    ...
    var f = value.eval(objectScope, componentScope);
    item[signalName].connect(item, f);
    return f;
}

global.connectSignal = connectSignal;
        ...
        if (i.indexOf("on") == 0 && i[2].toUpperCase() == i[2]) {
            ...
            if (!connectSignal(item, signalName, value, objectScope, componentScope)) {
                if (item.$setCustomData) {
                    item.$setCustomData(signalName, value, objectScope, componentScope);
                }
            }
            continue;
        }
